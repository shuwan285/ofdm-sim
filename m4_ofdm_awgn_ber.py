# -*- coding: utf-8 -*-
"""
M4 加信道 —— 第一步：OFDM + AWGN 噪声，跑 BER 曲线

关键结论：纯 AWGN 下，OFDM 的误码率 = 底层 QPSK 的误码率。
因为 IFFT/FFT 是线性变换，不改变噪声的统计特性。
（图里 OFDM 曲线和 QPSK 曲线几乎完全重合，就是证明）
"""
import numpy as np
import matplotlib
...
def ofdm_tx(bits):
    ...
    freq_grid = freq_sym.reshape(n_syms, N)              # 每行 = 一个 OFDM 符号
    time_grid = np.fft.ifft(freq_grid, axis=1)           # 每行 IFFT（打包）
    with_cp = np.concatenate([time_grid[:, -cp:], time_grid], axis=1)  # 加 CP
    return with_cp.flatten(), n_syms

def ofdm_rx(signal, n_syms):
    with_cp = signal.reshape(n_syms, N + cp)
    time_grid = with_cp[:, cp:]                          # 去 CP
    freq_grid = np.fft.fft(time_grid, axis=1)            # FFT（拆包）
    return qpsk_demod(freq_grid.flatten())

# ---------- 直传 QPSK（不加 OFDM，用于对比） ----------
def qpsk_direct_ber(bits, EbN0_db):
    symbols = qpsk_mod(bits)
    ber = []
    for eb in EbN0_db:
        eb_lin = 10 ** (eb / 10)
        n0 = (1 / 2) / eb_lin                            # QPSK 每比特能量 Eb=1/2
...
def ofdm_ber(bits, EbN0_db):
    ...
    n_rx = n_syms * N * 2                                # 实际恢复的比特数
    ber = []
    for eb in EbN0_db:
        eb_lin = 10 ** (eb / 10)
        # OFDM 噪声算法里多了个 1/N，因为 IFFT 把能量摊到了 N 个时域点上
        sigma2 = 1 / (4 * N * eb_lin)
